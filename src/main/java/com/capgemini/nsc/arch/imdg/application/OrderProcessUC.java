...
import com.capgemini.nsc.arch.imdg.details.processing.OrderHazelcastEntryProcessor;
...
public class OrderProcessUC {
	...
	/**
	 * Use Case for processing orders using Hazelcast and entry processor method
	 * 
	 * @param numberOfOrdersToProcess
	 * @param timerName 
	 * @return count of processed {@link Order}s
	 */
	public int processWithHazelcastAndEntryProcessor(int numberOfOrdersToProcess, String timerName) {
		return new OrderProcess(
				new HazelcastOrderRepository(), 
				new OrderHazelcastEntryProcessor() {})
			.process(numberOfOrdersToProcess, timerName);
	}
